...
tzs.forEach( tz => {
  console.log("====" + tz)
  process.env.TZ = tz; 
  console.log(dates.map(date=> DateTime.fromISO(date).toISO()));
});
...
[tz1,tz2,tz3].forEach( tz => {
  console.log("====" + tz)
  process.env.TZ = tz; 
  console.log(dates.map(date=> DateTime.fromISO(date + "+08:00").toISO()))
});
...
tzs.forEach(tz=>{
  ...
  process.env.TZ = tz;
  console.log(
    dates.map(d=> DateTime.fromISO(d+"+08:00"))
        .map(date=> date.toFormat("yyyy-MM-dd HH:mm:ss"))
  );
});
...
tzs.forEach(tz=>{
  ...
  console.log(
    dates.map(d=> DateTime.fromISO(d+"+08:00"))
        .map(date=> date.setZone(tz2).toFormat("yyyy-MM-dd HH:mm:ss"))
  );
});